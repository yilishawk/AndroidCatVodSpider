...
public class DuBoKu extends Spider {
    ...
    public String categoryContent(String tid, String pg, boolean filter, HashMap<String, String> extend) {
        try {
            ...
            // URL 拼接: /vodshow/{tid}-{area}-{by}-{class}-{lang}----{pg}---{year}.html
            ...
            for (Element li : items) {
                ...
                if (picText != null) remarks = picText.text().trim();
    ...
    public String detailContent(List<String> ids) {
        try {
            ...
            if (sketch != null) content = sketch.text().trim();
            ...
            Elements dataPs = detail.select("p.data");
            for (Element p : dataPs) {
            ...
            // 播放列表
            Elements episodes = doc.select(".myui-content__list li a");
            StringBuilder playUrl = new StringBuilder();
            for (Element a : episodes) {
                String name = a.text().trim();
                String epHref = a.attr("href");
                if (playUrl.length() > 0) playUrl.append("#");
                playUrl.append(name).append("$").append(epHref);
            }
    ...
    public String searchContent(String key, boolean quick) {
        try {
            ...
            String html = fetch(url);
            Document doc = Jsoup.parse(html);
            Elements items = doc.select("#searchList li");
            ...
            for (Element li : items) {
                ...
                if (tag != null) remarks = tag.text().trim();
    ...
    public String playerContent(String flag, String id, List<String> vipFlags) {
        try {
            ...
            String html = fetch(url);
            // 提取 "url":"..."
            Matcher m = Pattern.compile("\"url\"\\s*:\\s*\"([^\"]+)\"").matcher(html);
            if (m.find()) {
                String encUrl = m.group(1);
                // 补全 Base64 填充
                int padding = 4 - encUrl.length() % 4;
                if (padding != 4) {
                    for (int i = 0; i < padding; i++) encUrl += "=";
                }
                byte[] decoded = Base64.getDecoder().decode(encUrl);
                String realUrl = java.net.URLDecoder.decode(new String(decoded, StandardCharsets.UTF_8), "UTF-8");
