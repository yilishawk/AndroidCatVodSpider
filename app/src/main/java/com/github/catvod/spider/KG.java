...
public class KG extends Spider {
    ...
    public void init(Context context, String extend) {
        try {
            ...
            // 新增：支持 force_pc_ua
            if (rule.optBoolean("force_pc_ua", false)) {
                KaiGeNet.switchUA(false);
                logger("🔄 已切换为 PC User-Agent");
            }
    ...
    public String categoryContent(String tid, String pg, boolean f, HashMap<String, String> e) {
        try {
            ...
            if (pg.equals("1")) {
                KaiGeNet.smartRequest(this.siteUrl, "get", this.siteUrl, null, getHeaders(null), 2);
                try { Thread.sleep(200); } catch (Exception ignored) {}
            }

            OkResult res = KaiGeNet.smartRequest(this.siteUrl, method, url, body, getHeaders(null), 3);
            ...
            if (res.getCode() != 200 || TextUtils.isEmpty(html) || html.length() < 300) {
                ...
                res = KaiGeNet.smartRequest(this.siteUrl, method, url, body, getHeaders(null), 3);
                html = res.getBody();
            }
        ...
        try {
            ...
            for (int i = 0; i < stepCount; i++) {
                ...
                OkResult res = KaiGeNet.smartRequest(this.siteUrl, method, stepUrl,
                        ...
                        getHeaders(step.optJSONObject("headers")), 3);
    ...
    private String extract(Object root, String ruleStr) {
        try {
            ...
            if (realRule.startsWith("json:")) {
                return KaiGeEngine.doExtract(root.toString(), realRule, this.siteUrl).value;
            }
            ...
            if (realRule.contains("&&") || realRule.contains("[") || realRule.contains("json:")) {
