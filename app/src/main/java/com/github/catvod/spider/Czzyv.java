...
public class Czzyv extends Spider {
    ...
    private Map<String, String> getHeaders(String referer) {
        ...
        // 关键：将 WebView 刷出的 Cookie 实时同步给 OkHttp
        try {
    ...
    // ──────────────────────────────────────────────
    // 雷池/CF 绕过与请求代理的核心（方案 A：WebView Fetch）
    // ──────────────────────────────────────────────
    ...
    /**
     * 当 OkHttp 被阻断时，直接让 WebView 在后台发起 JS Fetch 请求，完全对齐 Chrome 指纹
     */
    private String getViaWebView(String targetUrl, String referer) {
        ...
        Init.run(() -> {
            ...
            String js = "(function() {" +
                    ...
                    "  })" +
                    "  .then(res => res.text())" +
                    "  .then(text => window.androidResult = text)" +
                    "  .catch(err => window.androidResult = 'ERROR: ' + err);" +
                    "})();";
            ...
            Runnable checkRunnable = new Runnable() {
                ...
                public void run() {
                    ...
                    sharedWebView.evaluateJavascript("window.androidResult", value -> {
                        if (value != null && !value.equals("null") && !value.equals("\"\"")) {
                            ...
                            result[0] = res;
                            latch.countDown();
                        } else if (attempts < 24) { // 24 * 250ms = 6秒
                            handler.postDelayed(this, 250);
                        } else {
                            logger("⏰ WebView fetch 等待超时");
                            latch.countDown();
                        }
                    });
                }
    ...
    private String get(String url, String referer) {
        ...
        // 核心检测：一旦 OkHttp 请求被雷池拦截或返回空，立刻切回 WebView fetch
