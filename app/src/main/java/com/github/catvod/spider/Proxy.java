...
public class Proxy {
    ...
    private static String proxifySegments(String m3u8Content) {
        ...
        // 段有三种形态都要改写：
        //   1) https://ipfs.ppnix.com/ipfs/<cid>          (无扩展名，实测主流)
        //   2) https://ipfs.ppnix.com/.../xxx.ts|.m4s|.mp4|.key
        // 用"路径段以 / 或行尾结束"来切，cid 段不带 . 后缀也能命中
        Pattern pattern = Pattern.compile("(https?://ipfs\\.ppnix\\.com/[^\\s'\"\\)]+)");
        ...
        try {
            while (matcher.find()) {
                ...
                // Java8: 用 replaceFirst + Matcher.quoteReplacement 避免 3 参 replace 重载(Java11)
                String candidateUrl = "https://" + randomNum + ".ppnix.com" +
                        matcher.group(1).replaceFirst("(?i)^https?://ipfs\\.ppnix\\.com", "");
        ...
        log("✅ proxifySegments 改写段数: " + count);
        return out.toString();
    }
    ...
    private static java.util.List<String> buildSegmentCandidates(String originalUrl) {
        ...
        // 匹配任意 *.ppnix.com 下的 /ipfs/<cid> 或 .ts/.m4s 段（cid 无扩展名）
    ...
    // ====================== PPnix AES Key ======================
    /**
     * 关键修正：www.ppnix.com/info/m3u8/key 返回的是 32 个【可打印明文字符串】
     * （如 "ba9bf05693b9fa202d922dd43a08f281"），它本身就是 AES 密钥的明文形式，
     * 前 16 个字符 = AES-128 key。它【不是】十六进制编码的 key。
     * 旧实现误用 hexStringToByteArray(把每个字符当 16 进制解 2 字节=1 字节)，
     * 得到错误 key，播放器解密全部失败(黑屏)。正确做法：直接取前 16 字节。
     */
    private static Object[] handlePpnixKey(Map<String, String> params) {
        try {
            ...
            headers.put("Referer", "https://www.ppnix.com/");
            headers.put("Origin", "https://www.ppnix.com");
            String keyStr = OkHttp.string(keyUrl, headers);
            if (keyStr == null || keyStr.trim().isEmpty()) {
                log("❌ ppnixKey 获取失败：返回空");
                return errorResponse(500, "key empty");
            }
            keyStr = keyStr.trim();
            if (isCloudflareChallenge(keyStr)) {
                log("🛡️ ppnixKey 疑似被 Cloudflare 拦截（返回的是挑战页而非 key），前200字符: "
                        + keyStr.substring(0, Math.min(200, keyStr.length())));
                return errorResponse(403, "blocked by cloudflare challenge");
            }
            // key 是明文：前 16 字节即 AES-128 key（按 UTF-8 字节取，非 16 进制解析）
            byte[] full = keyStr.getBytes("UTF-8");
            byte[] keyBytes;
            if (full.length >= 16) {
                keyBytes = new byte[16];
                System.arraycopy(full, 0, keyBytes, 0, 16);
            } else if (full.length > 0) {
                // 不足 16 字节时补 0（实测不会发生，防御）
                keyBytes = new byte[16];
                System.arraycopy(full, 0, keyBytes, 0, full.length);
            } else {
                log("❌ key 内容为空");
                return errorResponse(500, "invalid key");
            }
            log("🔑 获取到 key(明文前16B hex): " + bytesToHex(keyBytes)
                    + "  原始长度=" + full.length + "  前200: " + keyStr.substring(0, Math.min(200, keyStr.length())));
