# 爬蟲 API 規格說明（SPIDER.md 完整整理版）

本文件說明如何實作一個 Spider 爬蟲，包含所有方法的參數、回傳格式及 JSON 結構定義。

來源：https://github.com/FongMi/TV/blob/fongmi/docs/SPIDER.md
...
Spider 是應用程式爬蟲的抽象基底類別，位於 com.github.catvod.crawler.Spider。
每個影片來源（Site）對應一個 Spider 實例。
...
4    | HTTP URL        | 內建 JSON+Base64 ext  | 同 type=1，擴充參數以 Base64 編碼傳遞（ext=）。
...
參數：
- tid（String）：分類 ID，對應 Class.type_id。
- pg（String）：頁碼，從 "1" 開始。
...
參數：
- ids（List<String>）：影片 ID 清單，通常只含一個元素，對應 Vod.vod_id。
...
注意：若 Site.quickSearch = 0，快速搜尋會被跳過，直接回傳空結果。


--------------------------------------------------------------------------------
7. playerContent — 播放解析
--------------------------------------------------------------------------------
...
--------------------------------------------------------------------------------
11. manualVideoCheck / isVideoFormat — 影片格式判斷
--------------------------------------------------------------------------------
...
【homeContent】
- class：array<Class>  分類列表
- filters：object      篩選器定義，key 為 type_id，value 為 Filter 陣列

【homeVideoContent / categoryContent / detailContent / searchContent】
- list：array<Vod>     影片卡片列表
- pagecount：integer   總頁數（categoryContent、searchContent 使用）

【playerContent】
- url：string          實際播放媒體 URL
- parse：integer       0 = 直接播放，1 = 需進一步解析（預設 0）。jx=1 效果相同
- jx：integer          同 parse=1
- playUrl：string      解析器前綴或指定
- key：string          來源 key，用於從配置查找對應 Site.click
- click：string        點擊攔截處理 URL
- code：integer        非零時抑制 msg 顯示
- header：object       播放請求的額外 HTTP 標頭
- flag：string         播放來源旗標名稱，覆蓋原始 flag 參數
- jxFrom：string       強制指定解析器旗標
- format：string       媒體 MIME type（如 "application/x-mpegURL"）
- danmaku：array<Danmaku>  彈幕資料列表
- subs：array<Sub>     字幕列表
- drm：Drm             DRM 版權保護設定
...
vod_play_url      | string   | 播放集數 URL，格式詳見下方說明
...
cate              | Cate     | 資料夾顯示樣式物件，包含 land、circle、ratio 三個子欄位。設定此欄位等同於 vod_tag: "folder"
land              | integer  | 橫向顯示旗標，覆蓋 Class 層級的 land 設定
circle            | integer  | 圓形顯示旗標，覆蓋 Class 層級的 circle 設定
ratio             | float    | 卡片寬高比，覆蓋 Class 層級的 ratio 設定
style             | Style    | 此影片卡片的顯示樣式覆蓋
...
爬蟲可在回傳的媒體 URL 中使用 proxy:// 協議，將請求導向本地代理伺服器，由對應語言的 proxy() 方法處理。這樣可以在播放器無法直接存取來源時，讓爬蟲居中轉發資料。
